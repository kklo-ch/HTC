...
def process(G):
    ...
    hight, y_vector = tree(G, k=parameter, cs_dict=his)
    # print(y_vector)
    # print(hight)
    c = 0
    ...
    tarank_list = list()
    tarank_rank = list()
    final1 = sorted(final.items(), key=lambda x: x[0], reverse=False)
    for each in final1:
        tarank_list.append(each[1])
    final2 = sorted(final.items(), key=lambda x: x[1], reverse=True)
    for each in final2:
        tarank_rank.append(each[0])
    return tarank_list, tarank_rank
